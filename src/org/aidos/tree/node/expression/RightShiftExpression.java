package org.aidos.tree.node.expression;

import org.aidos.tree.node.PushInstruction;

/**
 * Represents a right shift expression.
 * @author `Discardedx2
 */
public class RightShiftExpression extends ArithmeticExpressionNode {

	/**
	 * Constructs a new {@link RightShiftExpression}.
	 * @param left The integer on the left hand assignment.
	 * @param right The integer on the right hand assignment.
	 */
	public RightShiftExpression(PushInstruction left, PushInstruction right) {
		super(left, right, Operator.RIGHT_SHIFT);
	}

	@Override
	public void invoke() {
		invokedValue = ((PushInstruction) left).getValue() >> ((PushInstruction) right).getValue();
	}

}
